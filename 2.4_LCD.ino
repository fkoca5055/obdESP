...
float BATTERY, turboPRESS, turboMAX, SMC;
...
void setup() {
  ...
  for (row=0; row<h; row++) {
   ...
   tft.drawLine(106, 0, 106, 110, MAGENTA); //DİKEY 1
   tft.drawLine(212, 0, 212, 110, MAGENTA); //DİKEY 2
...
void getSMC(){  
  ...
  OBD_read();
  SMC = ((strtol(&rxData[15],0,16)*256)+strtol(&rxData[18],0,16));  //((A*256)+B)/1000
  SMC = SMC / 100;
       /*if (BATTERY<12){
       tft.setTextColor(ILI9341_BLACK,ILI9341_YELLOW);
       }
